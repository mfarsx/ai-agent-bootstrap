---
name: init-memory
description: Populate the memory-bank files for a new or existing project so the agent has a complete context baseline. Use at the start of a project, when memory-bank is empty, or when the user asks to bootstrap, initialize, or set up the memory bank.
---
